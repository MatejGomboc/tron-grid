...
AllocatedImage Allocator::createImage3D(uint32_t width, uint32_t height, uint32_t depth, VkFormat format, VkImageUsageFlags usage) const
{
    VkImageCreateInfo image_info{};
    image_info.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
    ...
    image_info.arrayLayers = 1;
    image_info.samples = VK_SAMPLE_COUNT_1_BIT; // 3D images cannot be multisampled.
    image_info.tiling = VK_IMAGE_TILING_OPTIMAL;
    image_info.usage = usage;
    image_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    image_info.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
    ...
    VkResult result{vmaCreateImage(m_allocator, &image_info, &alloc_create_info, &image, &allocation, nullptr)};
    if (result != VK_SUCCESS) {
        m_logger.logFatal("Failed to create 3D VMA image.");
        std::abort();
    }
